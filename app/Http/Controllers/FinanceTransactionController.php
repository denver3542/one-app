...
class FinanceTransactionController extends Controller
{
    ...
    public function migrateTransactionToAccounting(Request $request)
    {
        $user_id = auth()->user()->user_id;
        ...
        join('finance_phase', 'finance_transaction.val_phase_id','=','finance_phase.phase_id')
        ->join('task', 'finance_transaction.val_assign_to','=','task.task_id')
        ->where('val_phase_id', $phase_id)
        ->where('val_assign_to', $task_id)
        // ->toSql();
        ->first();

        $items = FinanceField::where('finance_phase_id',$phase_id)->get();
        foreach ($items as $i => $item) {
            $item_id = $item->finance_id;
            $amount = $item->finance_value;
	...
	public function sortTransactionFees(Request $request)
    {
        // Get the sorted requirements from the request
        $sortedRequirements = $request->input('fees');
