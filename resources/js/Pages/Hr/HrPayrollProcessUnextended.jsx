...
const HrPayrollProcessUnextended = () => {
  const queryParameters = new URLSearchParams(window.location.search)
  const [searchParams, setSearchParams] = useSearchParams()
  const [selectMonth, setSelectMonth] = useState(searchParams.get('month'))
  const [selectFrom, setselectFrom] = useState(searchParams.get('from'))
  const [selectTo, setselectTo] = useState(searchParams.get('to'))
  ...
  useEffect(() => {
    filterPayroll(selectMonth, selectFrom, selectTo, selectYear, selectCutoff)
  }, [selectMonth, selectFrom, selectTo, selectYear, selectCutoff])

  const filterPayroll = async (month_val, from_val, to_val, year_val, cutoff_val) => {
    let dates = []
    dates = [month_val, from_val, to_val, year_val, cutoff_val]
    await axiosInstance.get(`/payroll/unextended/${dates.join(',')}`, { headers })
      .then((response) => {
  ...
  return (
    <Layout>
      ...
      <div className='block'>
                  ...
                  .map((payrollList, index) => {
                    return (
                      <TableRow key={index} hover
                        ...
                        <TableCell>{moment(payrollList.fromDate).format('MMM.DD') + ' - ' + moment(searchParams.get('month')).format('MMM.' + searchParams.get('to'))}</TableCell>
        ...
        {openPayroll && (
          <PayrollModal open={openPayroll} close={handleCloseModal} data={payrollData} cutoff={selectCutoff} type={1} processtype={1} />
        )}
...
export default HrPayrollProcessUnextended