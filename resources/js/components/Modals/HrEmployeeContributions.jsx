...
const HrEmployeeContributions = ({ open, close }) => {
  const [listContribution, setListContribution] = useState([]);
  const [listAddtionalContri, setListAddtionalContri] = useState([]);
  ...
  useEffect(() => {
    axiosInstance.get(`/additional_benefits/${3}`, { headers }).then((response) => {
      setListAddtionalContri(response.data.data);
    });
  }, [])

  useEffect(() => {
    axiosInstance.get('/contribution', { headers }).then((response) => {
      setListContribution(response.data.contribution);
    });
  }, [reducerValue])
  ...
  const handleSubmitBenefits = (e) => {
    ...
    new Swal({
      title: "Are you sure?",
      text: "You want to add this Contribution?",
      icon: "warning",
      dangerMode: true,
    }).then(res => {
      if (res.isConfirmed) {
        axiosInstance.post('/add_contribution', {
          benefitlist_id: contributionData.list_id,
  ...
  const handleDeleteContribution = (id) => {
    ...
    new Swal({
      ...
      if (res.isConfirmed) {
        axiosInstance.post('/delete_contribution', { benefits_id: id }, { headers })
          .then((response) => {
    ...
    <>
      <Dialog sx={{
        ...
        <Box className="d-flex justify-content-between" >
          <DialogTitle>
            <Typography className="text-center">Add new Contribution</Typography>
          </DialogTitle>
        ...
        <DialogContent>
              ...
              <FormGroup row={true} className="d-flex justify-content-between" sx={{
                ...
                <FormControl sx={{
                  ...
                  <Select
                    ...
                    label="Status"
                    value={contributionData.list_id}
                    onChange={(e) => setContributionData({ ...contributionData, list_id: e.target.value })}
                    notched={true}
                    sx={{ width: '100%', }}
                  >
                    {listAddtionalContri.map((list, index) => (
                ...
                <FormControl sx={{
                  ...
                  <input id="demo-simple-select" className='form-control' type="number" value={contributionData.amount}
                    onChange={(e) => setContributionData({ ...contributionData, amount: e.target.value })} style={{ height: 52 }} />
                </FormControl>
              </FormGroup>
          ...
          <TableContainer>
            <Table className="table table-md  table-striped  table-vcenter">
              ...
              <TableBody>
                {listContribution.map((res, index) => {
                  return (
                    <TableRow key={index}>
                      ...
                      <TableCell className='text-center'>
                        <Button
                          ...
                          variant="text"
                          style={{ color: 'red' }}
                          onClick={() => handleDeleteContribution(res.benefits_id)}
                        >
...
export default HrEmployeeContributions