    ...
    return (
        <div onClick={() => openSideNav()} >
            <a data-toggle="nav-submenu" href="#">
                <i className={items.icon} style={{ color: '#fc1414' }}></i><span className="sidebar-mini-hide">{items.text}
                </span>
                ...
                return (
                        <NavLink
							...
							// 	(isActive ? { backgroundColor: 'green', color: 'white' } : {})}
                                key={index}
						>
							<i style={{ color: '#fc1414' }}></i><span className="sidebar-mini-hide">{link.text}</span>
						</NavLink>
                )
            })}
        </div>
